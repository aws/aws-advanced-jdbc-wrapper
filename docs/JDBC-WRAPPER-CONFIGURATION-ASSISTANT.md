...
- **Do not restrict node selection in the target driver.** Parameters such as the PostgreSQL JDBC driver's `targetServerType=primary` make the driver reject a node by role. The wrapper strips these from its monitoring connections (a `WARNING` is logged when it does), because the green node is a replica until promotion and the monitor would otherwise never connect — leaving traffic held until `bgSwitchoverTimeoutMs` elapsed. Let the wrapper handle host selection instead. On versions without this behavior, override it per monitor with `blue-green-monitoring-targetServerType=any`.

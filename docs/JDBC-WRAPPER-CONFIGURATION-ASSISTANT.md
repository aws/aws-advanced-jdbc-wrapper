...
2. **How are reader/writer hosts discovered?**
   - *From cluster topology* (Aurora cluster or RDS Multi-AZ DB cluster) → a **topology** code (the `readWriteSplitting` family).
   - *From two fixed endpoints* you provide (community DB, RDS Proxy, custom routing; no topology) → a **`*Simple*`/`srw`** code (needs `srwReadEndpoint` + `srwWriteEndpoint`). Not for RDS Single-AZ.
   - *Aurora Global Database, home-region aware* (constrain connections to a home region, optionally Global Write Forwarding) → add the **`gdb*`** prefix (works with either topology or fixed endpoints).
3. **Who should decide reader vs writer — the app or the driver?**
   - *The application, by signalling read intent* via `setReadOnly(true)` or Spring `@Transactional(readOnly=true)` → a **manual** code.
   - *The driver, per statement, by inspecting SQL* (`SELECT` → reader, writes → writer) → an **`auto*`** code. Load `sqlParser` before it and add the `jsqlparser` dependency. Best when you can't or don't want to change application code.

   → Combine the answers to questions 2 and 3 (plus the `gdb*` prefix) to land on the exact code from the §5.12a table — e.g. topology + driver-decides = `autoReadWriteSplitting`; fixed endpoints + GDB + driver-decides = `gdbAutoSimpleReadWriteSplitting`. Confirm the chosen code back to the user.
...
4. **Reader load balancing — how should reads be spread across readers?** (see §5.12b and §17.3a):
   ...
   2. **(topology codes only) Sticky reader or fresh reader per query?** Default is *sticky*: one reader is chosen when the connection enters a read-only phase and reused. Set `queryLevelLoadBalancing=true` for *per-query* balancing — a fresh reader is chosen on each read-routing decision. Per-query spreads load more evenly but rotates connections more and reduces per-reader cache locality. Applies to the topology codes (`readWriteSplitting`, `autoReadWriteSplitting`, `gdbReadWriteSplitting`, `gdbAutoReadWriteSplitting`); the `*Simple*`/`srw` codes use a single fixed read endpoint, so there is no reader pool to balance.
   3. **(topology codes only) Should the writer also serve balanced reads?** `loadBalancingIncludeWriter=true` adds the writer to the reader-balancing pool (only meaningful with per-query balancing). Default is readers-only.
   4. **Re-executed statements across a switch:** `allowStatementRecreationOnConnectionSwitch` (family-wide, on by default) re-creates a re-executed `Statement`/`PreparedStatement`/`CallableStatement` on the newly selected node so it follows the routing, replaying recorded settings and bound parameters. A one-shot stream/LOB parameter or a pending batch can't be re-created, so the wrapper keeps it on its original connection and warns once. Only suggest turning it off if the user reports a problem with statement re-creation.
...
>
> **Jackson Databind (Secrets Manager and Okta plugins):** the driver is a multi-release JAR, so the required Jackson version depends on the Java runtime. On **Java 17+** these plugins use **Jackson 3.x** — add [`tools.jackson.core:jackson-databind`](https://central.sonatype.com/artifact/tools.jackson.core/jackson-databind) (tested against `3.2.1`). On **Java 8–16** they use **Jackson 2.x** — add [`com.fasterxml.jackson.core:jackson-databind`](https://central.sonatype.com/artifact/com.fasterxml.jackson.core/jackson-databind). The two use different groupIds/packages and coexist without conflict, so Jackson 3.x will not collide with a Jackson 2.x that Spring Boot 3.x (or another framework) already provides. See the [Jackson 3 Migration Guide](https://github.com/FasterXML/jackson/blob/main/jackson3/MIGRATING_TO_JACKSON_3.md).
...
- `iam` in the plugin list — required so **monitoring/topology** connections authenticate on IAM-only clusters. On wrapper **≤ 4.1.0**, declare plugins via the `wrapperPlugins` **string** (not only `ConfigurationProfile.withPluginFactories()`), or the `ctmi-*` monitor connection could drop `iam`; **fixed in 4.2.0** (PR #2021), where either declaration style works (issues #2020 / #1800; see §16.3).
...
- **Required Jackson dep (runtime-dependent):** **Jackson 3.x** (`tools.jackson.core:jackson-databind`, tested against `3.2.1`) on **Java 17+**, or **Jackson 2.x** (`com.fasterxml.jackson.core:jackson-databind`) on **Java 8–16**. On Java 17+ with only Jackson 2.x present, the plugin fails at initialization with `NoClassDefFoundError: tools/jackson/core/JacksonException`. Jackson 2.x and 3.x coexist (different groupIds/packages), so this does not conflict with a framework-provided Jackson 2.x.
- **Compatible with:** Aurora, Aurora Global, RDS Multi-AZ, RDS Single-AZ, RDS Proxy, Limitless.
...
- **Required deps:** `:rds`, `:sts`, `org.jsoup:jsoup`, plus Jackson Databind. Jackson is **runtime-dependent**: **Jackson 3.x** (`tools.jackson.core:jackson-databind`, tested against `3.2.1`) on **Java 17+**, or **Jackson 2.x** (`com.fasterxml.jackson.core:jackson-databind`) on **Java 8–16**. On Java 17+ with only Jackson 2.x present, the plugin fails at initialization with a `NoClassDefFoundError` for a `tools.jackson.*` class. Jackson 2.x and 3.x coexist (different groupIds/packages), so this does not conflict with a framework-provided Jackson 2.x.
...
There are eight read/write splitting codes. They share the same core (reader selection, session-state transfer, statement re-creation on connection switch, internal-pool reuse) and differ along three independent axes:

- **How a read is detected** — either the app toggles `Connection.setReadOnly(true/false)` (**manual**), or the `sqlParser` plugin classifies each statement automatically (`SELECT` → reader, DML/DDL → writer) — the **`auto*`** codes.
- **How reader/writer hosts are discovered** — from the cluster **topology** (Aurora / RDS Multi-AZ DB cluster), or from **two fixed endpoints** you supply (the **`*Simple*`/`srw`** codes — for community DBs, RDS Proxy, or custom routing).
- **Aurora Global Database awareness** — the **`gdb*`** codes add home-region-aware routing (and optional Global Write Forwarding) for Aurora Global Database.

Every combination of the three axes exists, so pick the code whose axes match your answers — don't reason from the name alone.

| Code | Read detection | Host discovery | Use when |
|---|---|---|---|
| `readWriteSplitting` | manual (`setReadOnly()`) | cluster topology | one datasource, app signals read intent, single-region cluster |
...
| `autoSimpleReadWriteSplitting` | automatic (SQL parsing) | two fixed endpoints | fixed endpoints, and you can't/won't call `setReadOnly()` |
...
| `gdbAutoReadWriteSplitting` | automatic (SQL parsing) | Aurora Global Database topology | Global Database, automatic routing |
| `gdbSimpleReadWriteSplitting` | manual (`setReadOnly()`) | GDB via two fixed endpoints | Global Database reached through fixed write/read endpoints |
| `gdbAutoSimpleReadWriteSplitting` | automatic (SQL parsing) | GDB via two fixed endpoints | Global Database, fixed endpoints, automatic routing |

> **Naming key:** `auto*` = SQL-parser routing (needs the `sqlParser` plugin + the `jsqlparser` dependency); `*Simple*`/`srw` = two fixed endpoints (needs `srwReadEndpoint` + `srwWriteEndpoint`); `gdb*` = Aurora Global Database home-region awareness. A code may stack all three (e.g. `gdbAutoSimpleReadWriteSplitting` = GDB + automatic + fixed endpoints).

Rules that hold for the whole family:

- Load exactly **one** of these codes (see §4.4). They are mutually exclusive.
- The **`auto*`** codes require the `sqlParser` plugin earlier in the chain and the `com.github.jsqlparser:jsqlparser` dependency on the classpath.
- The **`*Simple*`/`srw`** codes require the two endpoint properties (`srwReadEndpoint`, `srwWriteEndpoint`); the topology codes discover hosts instead.
- The **`gdb*`** codes accept the GDB home-region parameters from §5.12 and should only be used with Aurora Global Database.
- The internal connection pool (`connectionPoolType=hikari` or `c3p0`) is strongly recommended for all of them.

Per-query reader load balancing (`queryLevelLoadBalancing` / `loadBalancingIncludeWriter`) applies to the **topology** codes (`readWriteSplitting`, `autoReadWriteSplitting`, `gdbReadWriteSplitting`, `gdbAutoReadWriteSplitting`) — see §5.12b. The `*Simple*`/`srw` codes connect to a single fixed read endpoint, so there is no reader pool for the wrapper to balance. `allowStatementRecreationOnConnectionSwitch` (on by default, see §5.12b) applies family-wide. `readerHostSelectorStrategy` (§7) applies to whichever code selects a reader.

### 5.12b Reader load balancing and statement re-creation

**Per-query reader load balancing** (`queryLevelLoadBalancing` / `loadBalancingIncludeWriter`) is available on the **topology** codes — `readWriteSplitting`, `autoReadWriteSplitting`, `gdbReadWriteSplitting`, and `gdbAutoReadWriteSplitting`. The `*Simple*`/`srw` codes connect to a single fixed read endpoint, so there is no reader pool to balance. `allowStatementRecreationOnConnectionSwitch` applies to the whole family.

| Name | Default | Description |
|---|---|---|
| `queryLevelLoadBalancing` | `false` | By default the plugin picks one reader when the connection enters a read-only phase and stays on it (a "sticky" reader). With this enabled, it re-selects a reader on **each** read-routing decision, so consecutive reads can land on different readers. Reader selection still honors `readerHostSelectorStrategy` (use `roundRobin` for deterministic rotation). Routing is pinned while a transaction is open or autocommit is off, so balancing only happens between statements. (With `auto*` codes each `SELECT` is a routing point; with manual codes a routing decision happens on each `setReadOnly(true)`.) |
...
| `allowStatementRecreationOnConnectionSwitch` | `true` | Family-wide. When a routing decision switches the underlying connection to a different node, re-create an already-created `Statement`/`PreparedStatement`/`CallableStatement` on the new connection (replaying its recorded settings, bound parameters, and registered OUT parameters) so the query runs there. Set to `false` to leave reused statements on their original connection. When re-creation is impossible (a one-shot stream/LOB parameter or a pending batch), the wrapper falls back to the original connection and logs a one-time warning regardless of this setting. |

**How a switch is handled.** When routing selects a different node, the plugin switches the current connection to it (reusing a pooled connection when the internal pool is enabled, otherwise opening one). With `allowStatementRecreationOnConnectionSwitch` on (the default), a re-executed statement is re-created on the new connection and its recorded configuration and bound parameters are replayed, so it executes on the routed node. If the statement holds a one-shot stream/LOB parameter or a pending batch it cannot be re-created; the wrapper then keeps it on its original connection and warns once. Writes are never balanced: while a transaction is open or autocommit is off, the connection is pinned.

> **The read/write splitting plugin does not recover broken connections.** It routes reads and writes; it does not catch a connection error, reconnect, and retry the query. If a reader connection is dropped (e.g. the instance becomes unreachable), the failure surfaces to the caller — recovering from it is the job of the `failover`/`failover2` plugin (when configured) or the application. Some JDBC drivers report a dropped socket eagerly, in which case the plugin re-selects and can fall back to the writer; others surface it only on the next read. Either way the plugin never silently reconnects-and-retries.
...
> **This property is not exclusive to the deprecated plugin.** `skipInactiveWriterClusterEndpointCheck` is defined on `AuroraStaleDnsHelper`, and all three failover plugins (`failover`, `failover2`, `gdbFailover`) embed that helper and read the property on their `connect()` path. So it takes effect **even when `auroraStaleDns` is not in your plugin chain** — you do not need (and should not add) the deprecated plugin just to use it. If your chain includes `failover`, `failover2`, or `gdbFailover`, set the property directly. See §5.1 and §6.6.
...
- **Pre-switchover readiness signal (since 4.3.0):** the plugin logs a one-time `INFO` event, `[bgdId: '<id>'] Blue/Green target topology recognized`, as soon as it has both the blue and green topologies and the blue → green node mapping. It also prints that mapping and the resolved node IPs. Wait for this event before initiating a manual switchover — no non-default log level is required. On versions before 4.3.0 the same check requires `FINE`/`FINEST` logs — see §13.5.
- **Do not restrict node selection in the target driver.** Parameters such as the PostgreSQL JDBC driver's `targetServerType=primary` make the driver reject a node by role. Because the green node is a replica until promotion, the monitor would never connect, and traffic stays held until `bgSwitchoverTimeoutMs` elapses. Let the wrapper handle host selection instead. The wrapper strips these parameters from its monitoring connections and logs a `WARNING` when it does. On **4.3.0 and earlier** that handling does not exist — there, override the parameter per monitor with `blue-green-monitoring-targetServerType=any` (and `monitoring-`, `topology-monitoring-`, `frt-` for the other monitors).
- **Deleting and recreating a deployment (since 4.5.0):** a deployment can be deleted and a new one created without restarting the application. Once either the blue database has stopped reporting the deployment, or the green node has stopped reporting itself as part of it, for `bgDeploymentRemovedGraceMs`, the plugin logs an `INFO` event naming that environment, discards its collected state, and rediscovers the new green endpoint on its own. Allow up to `bgBaselineMs` after that, plus the RDS metadata provisioning time. The green-side signal matters because deleting a deployment does not necessarily make the green endpoint unreachable — a retained or renamed green instance keeps accepting connections while reporting no status. It is only used after green has reported the deployment at least once, so a green node still awaiting metadata provisioning is not mistaken for a deletion. Both signals apply only while the deployment is idle in `AVAILABLE`: from `SWITCHOVER_INITIATED` onwards both environments legitimately stop reporting (`collectStatus` filters out the separated `old1` endpoints), so resetting then would discard the blue → green routing mid-switchover and send connections to the old blue cluster. A stalled switchover is covered by `bgSwitchoverTimeoutMs`, and a deployment deleted mid-switchover is not detected — restart the application. On **4.4.0 and earlier** the plugin stays pinned to the deleted green endpoint indefinitely and its blue → green mapping keeps pointing at a node that is no longer part of any deployment; restart the application to recover.
- **An unreachable green environment blocks a switchover (since 4.5.0):** without a monitoring connection to green the blue → green mapping is never built, so a switchover has nothing to route to. After `bgEndpointUnreachableMs` the plugin logs a `WARNING` naming the endpoint and states that the deployment is not ready for switchover; the readiness event is withheld until green is reachable again. Resolve this before initiating a switchover. On **4.4.0 and earlier** the condition is only visible at `FINEST`, as a repeating `Opening monitoring connection` line with no matching `Opened` line.
...
| `bgDeploymentRemovedGraceMs` | `30000` | **Since 4.5.0.** How long the deployment must be continuously absent from the blue database's status information before it is treated as deleted and the monitoring context is reset. |
| `bgEndpointUnreachableMs` | `60000` | **Since 4.5.0.** How long a monitoring endpoint must be continuously unreachable before it is reported as unreachable and the deployment is reported as not ready for switchover. |
...
**Metadata provisioning timing:** The `bg` plugin detects a Blue/Green Deployment by polling the `mysql.rds_topology` table (MySQL) or `rds_tools.show_topology()` function (PostgreSQL) at the baseline interval (`bgBaselineMs`, default 60s). This metadata is provisioned by the RDS service asynchronously after BGD creation — the provisioning time varies and can range from minutes to many hours for standard RDS instances. The wrapper reports `NOT_CREATED` until the metadata appears. **Always verify the wrapper is ready before initiating a manual switchover** — on 4.3.0+ with default `INFO` logging, wait for the one-time pre-switchover readiness event `[bgdId: '<id>'] Blue/Green target topology recognized`, which the plugin emits once it has both blue and green topologies and the blue → green node mapping (it prints that mapping plus the resolved node IPs). No extra log configuration is needed. If the readiness event never appears despite the BGD showing "Available" in the AWS Console, contact AWS Support (RDS team) for service-side investigation.

**Verifying readiness on wrapper versions before 4.3.0.** There is no `INFO` readiness event, so raise `wrapperLoggerLevel` temporarily and read the plugin's own status logs (all from `BlueGreenStatusProvider` / `BlueGreenStatusMonitor`):

| Level | What to look for | Meaning |
|---|---|---|
| `WARNING` (default) | `[SOURCE] currentPhase: NOT_CREATED, exception while querying for blue/green status.` | The topology table/function isn't there yet (typically `SQLSyntaxErrorException`) — metadata is not provisioned. |
| `FINE` | `[bgdId: '1'] BG status: NOT_CREATED` → `BG status: CREATED` | Cheapest positive signal that the metadata has appeared. Note this line is **suppressed when `FINEST` is enabled** (the fuller summary below replaces it). |
| `FINEST` | `[SOURCE] (status not available) currentPhase: NOT_CREATED` | Per-role confirmation that metadata is still missing. |
| `FINEST` | `[bgdId: '1', role: TARGET] <interim status>` — non-empty `startTopology` / `hostNames` for **both** `SOURCE` and `TARGET` | Both sides' topology has been collected. |
| `FINEST` | `Corresponding nodes:` followed by `   <blue-node> -> <green-node>` rows with no `<null>` values | The blue → green mapping is built. This is the pre-4.3.0 equivalent of the 4.3.0 readiness table. |

Treat `CREATED` plus a fully populated `Corresponding nodes` block (both roles reporting topology) as ready. `FINEST` is verbose and may include connection metadata, so enable it only for the verification window and turn it back down before switchover; upgrading to 4.3.0+ avoids the tradeoff. You can also query the metadata directly via SQL (`SELECT * FROM mysql.rds_topology` / `SELECT * FROM rds_tools.show_topology()`) on any version, though that confirms the service side only, not that the wrapper has picked it up.
...
| Aurora topology-monitor thread (`ctmi-*`) fails IAM auth: `The server requested password-based authentication, but no password was provided by plugin null` | On wrapper **≤ 4.1.0**, plugins declared only via the class-based `ConfigurationProfile.withPluginFactories()` API did not propagate to the eagerly-created topology/monitoring connection, which fell back to the default plugin list (`initialConnection,auroraConnectionTracker,failover2,efm2`) and dropped `iam` (and other auth plugins) from the monitor's connect chain. **Fixed in 4.2.0** (PR #2021) — the profile now propagates. | Upgrade to **4.2.0+**, where `withPluginFactories()` works on its own. On **≤ 4.1.0**, work around it by also declaring plugins via the `wrapperPlugins` **string** (including `iam`, e.g. `initialConnection,failover2,readWriteSplitting,iam`) so monitoring connections include the auth plugin. Tracking: GitHub issues #2020 / #1800 |
| GDB inactive-cluster-writer endpoint connected/probed unexpectedly (TRACE shows `AuroraStaleDnsHelper | Stale DNS data detected. Opening a connection to ...`) | The `failover` / `failover2` / `gdbFailover` embedded stale-DNS helper is probing the inactive cluster writer (common with Global Write Forwarding) | Set `skipInactiveWriterClusterEndpointCheck=true` (§5.1 / §5.15). Note this is a **different** lever than `initialConnection`'s `inactiveClusterWriterEndpointSubstitutionRole` / `verifyInactiveClusterWriterEndpointConnectionType` |
...
| `NoClassDefFoundError: tools/jackson/core/JacksonException` (or another `tools.jackson.*` class) at startup, from `AwsSecretsManagerConnectionPluginFactory` / Okta credentials factory | Running on **Java 17+** with `awsSecretsManager`, `awsSecretsManager2`, or `okta`, but only **Jackson 2.x** (`com.fasterxml.jackson`) is on the classpath. On Java 17+ the multi-release JAR selects the **Jackson 3.x** (`tools.jackson`) implementation of these plugins, which Spring Boot 3.x does **not** provide. | Add **Jackson 3.x** — `tools.jackson.core:jackson-databind` (e.g. `3.2.1`). It has a different groupId/package from Jackson 2.x, so it coexists with the framework's Jackson 2.x without conflict — no need to change Spring Boot's Jackson. On Java 8–16, use Jackson 2.x (`com.fasterxml.jackson.core:jackson-databind`) instead. See the [Jackson 3 Migration Guide](https://github.com/FasterXML/jackson/blob/main/jackson3/MIGRATING_TO_JACKSON_3.md). |
...
| After a Blue/Green Deployment is deleted and recreated without restarting the application, the new green environment never receives topology probes (`rds_tools.show_topology` shows 0 calls in `pg_stat_statements`); the blue → green mapping is absent or still names the old green node. Depending on whether the old green endpoint still resolves, `FINEST` either repeats `[TARGET] Opening monitoring connection to <old green endpoint>` with no matching `Opened` line, or shows the target monitor connecting successfully while reporting `phase <null>` | The target monitor latched onto the green endpoint of the deleted deployment. Its fallback to the blue endpoint only applies before an endpoint has been discovered, so it never re-points, and the plugin only recreated its monitors after a *completed* switchover. If the old green instance was retained or its name still resolves, the monitor keeps connecting to a node that has been detached from the deployment. **Fixed in 4.5.0**: a deployment that either environment stops reporting for `bgDeploymentRemovedGraceMs` is treated as deleted, and the monitoring context is reset and rediscovered (logged at `INFO`) | Upgrade to **4.5.0+**. On earlier versions, restart the application; alternatively rename the new green instance to the DB identifier shown in the `Opening monitoring connection` log line so the cached endpoint resolves to it |
| Blue/Green switchover holds traffic and never builds a blue → green mapping, with no obvious error | The plugin cannot reach the green environment, so it never collects the green topology. Before **4.5.0** this is only visible at `FINEST` as a repeating `Opening monitoring connection` line with no matching `Opened` | On **4.5.0+** the plugin logs a `WARNING` after `bgEndpointUnreachableMs` and reports the deployment as not ready for switchover; resolve the reachability problem (security groups, direct access to instance endpoints) before switching over |
...
└─ One datasource that serves both reads and writes
   ...
   └─ Step 2 — how are reader/writer hosts discovered? (combine with Step 1)
      ...
      ├─ Two fixed endpoints (community DB, RDS Proxy, custom routing; needs `srwReadEndpoint` + `srwWriteEndpoint`)
      │     └─ manual → `srw`                            auto → `autoSimpleReadWriteSplitting`
      └─ Aurora Global Database (home-region aware) — add the `gdb*` prefix
            ├─ topology:        manual → `gdbReadWriteSplitting`        auto → `gdbAutoReadWriteSplitting`
            └─ fixed endpoints: manual → `gdbSimpleReadWriteSplitting`  auto → `gdbAutoSimpleReadWriteSplitting`
```

> Every path has both a manual (`setReadOnly()`) and an automatic (`auto*`, SQL-parser) form — on topology or fixed-endpoint host discovery, with or without the `gdb*` Global Database prefix — for eight codes in total. The `auto*` codes need the `sqlParser` plugin loaded earlier plus the `jsqlparser` dependency.
...
**Spreading reads across readers:** if you want each read to rotate across readers instead of sticking to one, add `queryLevelLoadBalancing=true` (the **topology** codes — `readWriteSplitting`, `autoReadWriteSplitting`, `gdbReadWriteSplitting`, `gdbAutoReadWriteSplitting`; pair with `readerHostSelectorStrategy=roundRobin` for deterministic rotation). See §5.12b and §17.3a.

### 17.3a Choosing reader load balancing behavior

Picking the read/write splitting *code* only decides how reads are detected and how hosts are discovered. How reads are *distributed across readers* is a separate set of choices. `readerHostSelectorStrategy` applies to whichever code selects a reader; per-query balancing (`queryLevelLoadBalancing` / `loadBalancingIncludeWriter`) applies to the **topology** codes (`readWriteSplitting`, `autoReadWriteSplitting`, `gdbReadWriteSplitting`, `gdbAutoReadWriteSplitting`) — the `*Simple*`/`srw` codes use a single fixed read endpoint, so there is no reader pool to balance. Walk these once the code is chosen:

```
On a topology code? Do you want reads spread across multiple readers within one read-only phase?
├─ No / on a `*Simple*`/`srw` code — one reader per read-only phase (default; best cache locality, fewest switches)
│     └─ Leave `queryLevelLoadBalancing=false` (default). Still choose HOW that reader is picked → `readerHostSelectorStrategy` (below).
└─ Yes (topology code) — rotate readers (better spread for many small reads / long-lived connections)
      ├─ Set `queryLevelLoadBalancing=true` — each read-routing decision re-selects a reader (switch happens between statements, never mid-transaction).
...
| Initiating Blue/Green switchover before the wrapper detects `CREATED` | The `mysql.rds_topology` table (MySQL) or `rds_tools.show_topology()` function (PostgreSQL) is provisioned by the RDS service asynchronously after BGD creation. Provisioning time varies (minutes to hours for standard RDS). If the switchover is initiated while the wrapper still reports `NOT_CREATED`, it cannot assist with the transition — connections experience full downtime instead of reduced switchover impact | Before initiating switchover: (1) verify BGD shows "Available" in AWS Console, (2) query the metadata directly (`SELECT * FROM mysql.rds_topology` or `SELECT * FROM rds_tools.show_topology()`) to confirm it's populated, (3) on 4.3.0+, wait for the wrapper's one-time readiness event at `INFO` — `[bgdId: '<id>'] Blue/Green target topology recognized` — which reports the blue → green node mapping; on earlier versions, temporarily raise the log level and wait for `BG status: CREATED` (`FINE`) plus a populated `Corresponding nodes` block (`FINEST`) — see §13.5. If the readiness event never appears despite "Available" status, contact AWS Support (RDS team) |
...
| On wrapper **≤ 4.1.0**, declaring plugins only via `ConfigurationProfile.withPluginFactories()` (class-based API) for an IAM-only / auth-required cluster | On those versions the eagerly-created topology/monitoring connection did not inherit the profile and fell back to the default plugin list, dropping `iam` — the `ctmi-*` monitor thread then failed auth (`no password was provided by plugin null`). Fixed in **4.2.0** (PR #2021). See issues #2020 / #1800 | Upgrade to **4.2.0+**, where `withPluginFactories()` propagates the profile to monitor connections and works on its own. On **≤ 4.1.0**, work around it by also declaring plugins (including `iam`) via the `wrapperPlugins` string |
...
| Adding the deprecated `auroraStaleDns` plugin just to get `skipInactiveWriterClusterEndpointCheck` | The property is already honored by `failover` / `failover2` / `gdbFailover` via the embedded `AuroraStaleDnsHelper`; adding the deprecated plugin is unnecessary and can conflict (it's mutually exclusive with `initialConnection`, `srw`, `gdbReadWriteSplitting`) | Just set `skipInactiveWriterClusterEndpointCheck=true`; keep your existing failover chain (§5.1 / §5.15) |
| Restricting node selection in the target driver, e.g. the PostgreSQL JDBC driver's `targetServerType=primary` | Host selection is the wrapper's job. Worse, the wrapper's internal monitoring connections inherited the setting and are deliberately opened against readers and replicas, so the driver rejected them and the dependent monitor silently stopped working. This is what caused issue #2096: the Blue/Green monitor could not connect to the green node (a replica until promotion), never observed the switchover finishing, and held all traffic until `bgSwitchoverTimeoutMs` elapsed. EFM, topology monitoring, and `fastestResponse` are affected the same way | Remove the parameter and let the wrapper select hosts (use `failoverMode` / a reader endpoint to express intent instead). The wrapper strips it from monitoring connections and logs a `WARNING`; on **4.3.0 and earlier** that handling does not exist, so override it per monitor there with `blue-green-monitoring-targetServerType=any`, `monitoring-targetServerType=any`, `topology-monitoring-targetServerType=any`, `frt-targetServerType=any` |

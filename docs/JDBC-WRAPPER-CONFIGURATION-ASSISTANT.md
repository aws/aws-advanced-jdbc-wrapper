...
- **Deleting and recreating a deployment (since 4.5.0):** a deployment can be deleted and a new one created without restarting the application. Once the blue database has stopped reporting the deployment for `bgDeploymentRemovedGraceMs`, the plugin logs an `INFO` event, discards its collected state, and rediscovers the new green endpoint on its own. Allow up to `bgBaselineMs` after that, plus the RDS metadata provisioning time. On **4.4.0 and earlier** the plugin stays pinned to the deleted green endpoint indefinitely and its blue → green mapping keeps pointing at nodes that no longer exist; restart the application to recover.
...
| After a Blue/Green Deployment is deleted and recreated without restarting the application, the new green environment never receives topology probes (`rds_tools.show_topology` shows 0 calls in `pg_stat_statements`); `FINEST` logs repeat `[TARGET] Opening monitoring connection to <old green endpoint>` with no matching `Opened` line; the blue → green mapping is absent or still names the deleted nodes | The target monitor latched onto the green endpoint of the deleted deployment. Its fallback to the blue endpoint only applies before an endpoint has been discovered, so it retries an endpoint that no longer resolves indefinitely, and the plugin only recreated its monitors after a *completed* switchover. **Fixed in 4.5.0**: a deployment that stops being reported for `bgDeploymentRemovedGraceMs` is treated as deleted, and the monitoring context is reset and rediscovered (logged at `INFO`) | Upgrade to **4.5.0+**. On earlier versions, restart the application; alternatively rename the new green instance to the DB identifier shown in the `Opening monitoring connection` log line so the cached endpoint resolves again |

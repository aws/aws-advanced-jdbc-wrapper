...
:exclamation: **NOTE**: Aurora Host List Connection Plugin is deprecated. The wrapper is able to automatically set the Aurora host list provider for connections to Aurora MySQL and Aurora PostgreSQL databases.
If you were using the Aurora Host List Connection Plugin, you can simply remove the plugin from the `wrapperPlugins` parameter.

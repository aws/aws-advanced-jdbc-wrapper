...
If you are connecting through an [Aurora custom endpoint](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Aurora.Endpoints.Custom.html) (`<custom-endpoint-name>.cluster-custom-<XYZ>.<region>.rds.amazonaws.com`), also enable the [Custom Endpoint Plugin](./using-plugins/UsingTheCustomEndpointPlugin.md) (plugin code `customEndpoint`) so that the wrapper only uses instances that are members of that custom endpoint. Note that an Aurora custom endpoint is not the same thing as a user custom domain (a CNAME alias pointing at an RDS endpoint); the latter is handled by the `clusterInstanceHostPattern` parameter instead. See [Database URL types compatibility](./CompatibilityEndpoints.md) for the full list of URL types.
...
[Exception context injection](./ExceptionContext.md) provides an independent mechanism for collecting diagnostic details during issue troubleshooting. Both the exception context feature and driver logging are essential for comprehensive analysis.
...
> [!IMPORTANT]\
> `wrapperLoggerLevel` is a convenience for simple applications, not a replacement for a logging
> configuration. It's applied on a best-effort basis and has the following limitations:
> - **A logger with an explicitly configured level is not affected.** The parameter sets the level
>   on the wrapper's parent logger, `software.amazon.jdbc`. The Java Util Logger uses the level of
>   the nearest logger that has one set, so entries like `software.amazon.jdbc.Driver.level=FINER`
>   in a `.properties` file take precedence over `wrapperLoggerLevel`. In particular,
>   `wrapperLoggerLevel=OFF` does **not** silence a logger whose level is configured in a
>   `.properties` file; remove or change that entry instead.
> - **It's applied when a connection is opened, and it's JVM-wide.** The level is set on a logger
>   shared by the whole JVM, so the most recently opened connection wins for all connections, and
>   the level stays in effect after that connection is closed. Anything logged before the first
>   connection is opened, for instance while the driver is being loaded, uses the level the
>   application has configured.
> - **It only adjusts a `ConsoleHandler` attached to the root logger**, and only to make it more
>   detailed. No handler is added, and no other handler type is modified. If the application removed
>   or replaced the default handlers, this parameter may produce no visible output at all.
> - **It's not authoritative when a 3rd party logging library is used.** See
>   [Using an External Logger](#using-an-external-logger) below.
>
> Use a `.properties` file or the logging configuration of the application for predictable results.
...
|---------------------------------------------------|-------------------------|-----------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------|
| `clusterId`                                       | 0.1.0 | `String`  | If connecting to multiple database clusters within a single application: Yes<br><br>Otherwise: No<br><br>:warning:If `clusterId` is omitted, you may experience various issues. | A unique identifier for the cluster. Connections with the same cluster id share a cluster topology cache. For applications that only use a single cluster, this parameter is optional and defaults to `1`. When supporting multiple database clusters, this parameter becomes mandatory. Each connection string must include the `clusterId` parameter with a value that can be any number or string. However, all connection strings associated with the same database cluster must use identical `clusterId` values, while connection strings belonging to different database clusters must specify distinct values. Examples of value: `1`, `2`, `1234`, `abc-1`, `abc-2`.<br><br> Please see [clusterId documentation](./ClusterId.md) for more information. | `1`            |
| `wrapperLoggerLevel`                              | 1.0.0 | `String`  | No                                                                                                                                                                              | Logger level of the AWS Advanced JDBC Wrapper. <br><br/>If it is used, it must be one of the following values: `OFF`, `SEVERE`, `WARNING`, `INFO`, `CONFIG`, `FINE`, `FINER`, `FINEST`, `ALL`. <br><br/>Applied on a best-effort basis to the `software.amazon.jdbc` logger when a connection is opened. It does not override a level that is explicitly configured for a particular logger. See [Using the Java Util Logger](#using-the-java-util-logger).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `null`         |
| `database`                                        | 0.1.0 | `String`  | No                                                                                                                                                                              | Database name.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `null`         |
| `user`                                            | 0.1.0 | `String`  | No                                                                                                                                                                              | Database username.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `null`         |
| `password`                                        | 0.1.0 | `String`  | No                                                                                                                                                                              | Database password.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `null`         |
| `wrapperDialect`                                  | 2.0.0 | `String`  | No                                                                                                                                                                              | Please see [this page on database dialects](./DatabaseDialects.md), and whether you should include it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | `null`         |
| `wrapperLogUnclosedConnections`                   | 1.0.0 | `Boolean` | No                                                                                                                                                                              | Allows the AWS Advanced JDBC Wrapper to capture a stacktrace for each connection that is opened. If the `finalize()` method is reached without the connection being closed, the stacktrace is printed to the log. This helps developers to detect and correct the source of potential connection leaks.                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `false`        |
| `loginTimeout`                                    | 2.2.0 | `Integer` | No                                                                                                                                                                              | Login timeout in milliseconds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `null`         |
| `connectTimeout`                                  | 2.3.1 | `Integer` | No                                                                                                                                                                              | Socket connect timeout in milliseconds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `null`         |
| `socketTimeout`                                   | 2.3.1 | `Integer` | No                                                                                                                                                                              | Socket timeout in milliseconds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `null`         |
| `tcpKeepAlive`                                    | 2.3.1 | `Boolean` | No                                                                                                                                                                              | Enable or disable TCP keep-alive probe.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | `false`        |
| `targetDriverAutoRegister`                        | 2.3.1 | `Boolean` | No                                                                                                                                                                              | Allows the AWS Advanced JDBC Wrapper to register a target driver based on `wrapperTargetDriverDialect` configuration parameter or, if it's missed, on a connection url protocol.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | `true`         |
| `transferSessionStateOnSwitch`                    | 2.3.3 | `Boolean` | No                                                                                                                                                                              | Enables transferring the session state to a new connection.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `true`         |
| `resetSessionStateOnClose`                        | 2.3.3 | `Boolean` | No                                                                                                                                                                              | Enables resetting the session state before closing connection.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `true`         |
| `rollbackOnSwitch`                                | 2.3.3 | `Boolean` | No                                                                                                                                                                              | Enables rolling back a current transaction, if any in effect, before switching to a new connection.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `true`         |
| `awsProfile`                                      | 2.3.3 | `String`  | No                                                                                                                                                                              | Allows users to specify a profile name for AWS credentials. This parameter is used by plugins that require AWS credentials, like the [IAM Authentication Connection Plugin](./using-plugins/UsingTheIamAuthenticationPlugin.md) and the [AWS Secrets Manager Connection Plugin](./using-plugins/UsingTheAwsSecretsManagerPlugin.md).                                                                                                                                                                                                                                                                                                                                                                                                                             | `null`         |
| ~~`enableGreenNodeReplacement`~~                  | 2.3.6 | `Boolean` | No                                                                                                                                                                              | **Deprecated. Use `bg` plugin instead.** Enables replacing a green node host name with the original host name when the green host DNS doesn't exist anymore after a blue/green switchover. Refer to [Overview of Amazon RDS Blue/Green Deployments](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/blue-green-deployments-overview.html) for more details about green and blue nodes.                                                                                                                                                                                                                                                                                                                                                                    | `false`        |
| `wrapperCaseSensitive`,<br>`wrappercasesensitive` | 2.6.0 | `Boolean` | No                                                                                                                                                                              | Allows the driver to change case sensitivity for parameter names in the connection string and in connection properties. Set parameter to `false` to allow case-insensitive parameter names.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `true`         |
| `skipWrappingForPackages`                         | 2.6.3 | `String`  | No                                                                                                                                                                              | Register Java package names (separated by comma) which will be left unwrapped. This setting modifies all future connections established by the driver, not just a particular connection.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `com.pgvector` |
| `connectionPoolType`                              | 3.1.0 | `String`  | No                                                                                                                                                                              | Activate internal connection pooling and set up an internal pool implementation type. Possible values are `hikari` and `c3p0`.<br><br>In order to configure specific pooling parameters use configuration property names for classes `com.zaxxer.hikariHikariConfig` or `com.mchange.v2.c3p0.ComboPooledDataSource` prefixed with `cp-`. For example, `connectionPoolType=hikari&cp-MaximumPoolSize=20&cp-MinimumIdle=1`. <br><br>Internal connection pool is maintained per database cluster specified by `clusterId` parameter. Clusters with different `clusterId` values will use separate connection pools.                                                                                                                                                 | `null`         |
...
| Property                                                 | Available Since Version | Value                                             | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Default Value              |
|----------------------------------------------------------|-------------------------|---------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------|
| `aws.jdbc.config.cleanup.thread.ttl`                     | 3.1.0 | `Long`                                            | Time-to-live in milliseconds for the cleanup thread that monitors unclosed connections. The thread will automatically stop after this period of inactivity to conserve resources. Only relevant when `wrapperLogUnclosedConnections` is enabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `30000` (30 seconds)       |
| `aws.jdbc.config.exception.context.enabled`              | 3.3.0 | boolean                                           | Enables injection of connection context into all exceptions. <br><br> Please see [exception context documentation](./ExceptionContext.md) for more information.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `true` (enabled)           |
| `aws.jdbc.config.exception.context.injection.type`       | 3.3.0 | `ADD_SUPPRESSED_EXCEPTION` or `INJECT_TO_MESSAGE` | Specifies how context should be injected into the main exception. When `ADD_SUPPRESSED_EXCEPTION` is selected, a connection context is added as a suppressed exception of type software.amazon.jdbc.exceptions.SnapshotStateException to the main exception. The suppressed exception has a message that represents the connection state snapshot and a list of recent important events. When `INJECT_TO_MESSAGE` is selected, a connection context is added at the end of the main exception message. No suppressed exception is added to the main exception in this case. Verify with your application's exception reporting systems and tools which option is most suitable. <br><br> Please see [exception context documentation](./ExceptionContext.md) for more information. | `ADD_SUPPRESSED_EXCEPTION` |
| `aws.jdbc.config.exception.context.driver.queue.ttl`     | 3.3.0 | long                                              | Time-to-live in milliseconds for collected important events at the driver level. <br><br> Please see [exception context documentation](./ExceptionContext.md) for more information.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `60000` (60 seconds)       |
| `aws.jdbc.config.exception.context.connection.queue.ttl` | 3.3.0 | long                                              | Time-to-live in milliseconds for collected important events at the connection level. <br><br> Please see [exception context documentation](./ExceptionContext.md) for more information.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `60000` (60 seconds)       |
| `aws.jdbc.config.host.cache.enabled`                     | 4.0.0 | boolean                                           | Enables caching of host identification results by the connection host name. When enabled, subsequent calls to identifyConnection for the same host will use the cached result instead of querying the database again.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `true`                     |
| `aws.jdbc.config.host.cache.regexp`                      | 4.4.0 | string                                            | A regular expression controlling which host names are eligible for host identification caching. Only hosts whose names match this regular expression will have their identification results cached or retrieved from the cache. <br><br> Note: the previously released property name `aws.jdbc.config.hast.cache.regexp` contained a typo and is now deprecated. It is still honored for backwards compatibility but will be removed in a future release; please migrate to `aws.jdbc.config.host.cache.regexp`.                                                                                                                                                                                                                                                                        | `.*` (any host name)       |

**Example:**
```bash
java -Daws.jdbc.config.cleanup.thread.ttl=60000 -jar myapp.jar
```
...
| Parameter                    | Available Since Version | Value     | Required | Description                                                                                                                                                                                          | Default Value                                              |
|------------------------------|-------------------------|-----------|----------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------|
| `wrapperPlugins`             | 1.0.0 | `String`  | No       | Comma separated list of connection plugin codes. <br><br>Example: `failover,efm2`                                                                                                                    | `initialConnection,auroraConnectionTracker,failover2,efm2` | 
| `autoSortWrapperPluginOrder` | 2.2.4 | `Boolean` | No       | Allows the AWS Advanced JDBC Wrapper to sort connection plugins to prevent plugin misconfiguration. Allows a user to provide a custom plugin order if needed.                                        | `true`                                                     | 
| `wrapperProfileName`         | 1.0.0 | `String`  | No       | Driver configuration profile name. Instead of listing plugin codes with `wrapperPlugins`, the driver profile can be set with this parameter. <br><br> Example: See [below](#configuration-profiles). | `null`                                                     |
...
| [GDB Failover Connection Plugin](./using-plugins/UsingTheGdbFailoverPlugin.md)                                       | `gdbFailover`                     | Aurora Global Database, limited support for Aurora and RDS Multi-AZ DB Cluster | Enables the failover functionality supported by Amazon Aurora Global Database. Allows setting complex failover modes depending on GDB primary region and the driver defined home region. It extends functionality of the Failover Connection Plugin v2 (`failover2`).                                                                                                                                                                                                                                                                                                                                                                                           | None                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 3.2.0                   |
...
| [Remote Query Cache Connection Plugin](./using-plugins/UsingTheRemoteQueryCachePlugin.md)                            | `remoteQueryCache`                | Any database                                                                   | Caches results from SQL queries in a remote valkey cache server with TTL defined in the prefixed SQL query hints                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | None                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 3.3.0                   |
| [Data Local Cache Connection Plugin](./using-plugins/UsingTheDataCachePlugin.md)                                                                                   | `dataCache`                       | Any database                                                                   | Caches results locally from SQL queries matching the regular expression specified in the  `dataCacheTriggerCondition` configuration parameter. Cached results are never invalidated by writes, so `dataCacheTtlMs` bounds how stale a served result can be.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | None                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 1.0.0                   |
...
| [IAM Authentication Connection Plugin](./using-plugins/UsingTheIamAuthenticationPlugin.md)                           | `iam`                             | Aurora, RDS Multi-AZ DB Cluster                                                | Enables users to connect to their Amazon Aurora clusters using AWS Identity and Access Management (IAM).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | [AWS Java SDK RDS v2.x](https://central.sonatype.com/artifact/software.amazon.awssdk/rds) and [valid AWS credentials](./AwsCredentials.md)                                                                                                                                                                                                                                                                                                                                                                                          | 1.0.0                   |
| [KMS Encryption Plugin](./using-plugins/UsingTheKmsEncryptionPlugin.md)                                              | `kmsEncryption`                   | Any database                                                                   | Enables transparent column-level encryption and decryption using AWS KMS for key management. Encrypts data before storage and decrypts on retrieval without application code changes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | [AWS Java SDK KMS v2.x](https://central.sonatype.com/artifact/software.amazon.awssdk/kms) and [valid AWS credentials](./AwsCredentials.md)                                                                                                                                                                                                                                                                                                                                                                                          | 4.0.0                   |
| [AWS Secrets Manager Connection Plugin](./using-plugins/UsingTheAwsSecretsManagerPlugin.md)                          | `awsSecretsManager`               | Any database                                                                   | Enables fetching database credentials from the AWS Secrets Manager service.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Jackson Databind ([`tools.jackson.core`](https://central.sonatype.com/artifact/tools.jackson.core/jackson-databind) on Java 17+, [`com.fasterxml.jackson.core`](https://central.sonatype.com/artifact/com.fasterxml.jackson.core/jackson-databind) on Java 8–16)<br>[AWS Secrets Manager](https://central.sonatype.com/artifact/software.amazon.awssdk/secretsmanager) and [valid AWS credentials](./AwsCredentials.md)                                                                                                             | 1.0.0                   |
| [AWS Secrets Manager Connection Plugin v2](./using-plugins/UsingTheAwsSecretsManagerPlugin2.md)                      | `awsSecretsManager2`              | Any database                                                                   | Enables fetching database credentials from the AWS Secrets Manager service using a Stale-While-Revalidate (SWR) caching strategy. Connections proceed immediately using cached credentials (even if stale), while a background async task refreshes the cache. This eliminates connection latency spikes during credential refresh on cache expiry and improves resilience during temporary Secrets Manager outages.                                                                                                                                                                                                                                            | Jackson Databind ([`tools.jackson.core`](https://central.sonatype.com/artifact/tools.jackson.core/jackson-databind) on Java 17+, [`com.fasterxml.jackson.core`](https://central.sonatype.com/artifact/com.fasterxml.jackson.core/jackson-databind) on Java 8–16)<br>[AWS Secrets Manager](https://central.sonatype.com/artifact/software.amazon.awssdk/secretsmanager) and [valid AWS credentials](./AwsCredentials.md)                                                                                                             | 4.2.0                   |
| [Federated Authentication Plugin](./using-plugins/UsingTheFederatedAuthPlugin.md)                                    | `federatedAuth`                   | Aurora, RDS Multi-AZ DB Cluster                                                | Enables users to authenticate using Federated Identity and then connect to their Amazon Aurora Cluster using AWS Identity and Access Management (IAM).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | [Jackson Databind](https://central.sonatype.com/artifact/com.fasterxml.jackson.core/jackson-databind)<br/>[AWS Java SDK RDS v2.7.x or later](https://central.sonatype.com/artifact/software.amazon.awssdk/rds) and [valid AWS credentials](./AwsCredentials.md)<br/>[AWS Java SDK STS v2.7.x or later](https://central.sonatype.com/artifact/software.amazon.awssdk/sts)                                                                                                                                                            | 2.3.2                   |
| [Okta Authentication Plugin](./using-plugins/UsingTheOktaAuthPlugin.md)                                              | `okta`                            | Aurora, RDS Multi-AZ DB Cluster                                                | Enables users to authenticate using Federated Identity and then connect to their Amazon Aurora Cluster using AWS Identity and Access Management (IAM).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Jackson Databind ([`tools.jackson.core`](https://central.sonatype.com/artifact/tools.jackson.core/jackson-databind) on Java 17+, [`com.fasterxml.jackson.core`](https://central.sonatype.com/artifact/com.fasterxml.jackson.core/jackson-databind) on Java 8–16)<br/>[AWS Java SDK RDS v2.7.x or later](https://central.sonatype.com/artifact/software.amazon.awssdk/rds) and [valid AWS credentials](./AwsCredentials.md)<br/>[AWS Java SDK STS v2.7.x or later](https://central.sonatype.com/artifact/software.amazon.awssdk/sts) | 2.3.6                   |
...
| [Automatic Read Write Splitting Plugin](./using-plugins/UsingTheAutoReadWriteSplittingPlugin.md)                     | `autoReadWriteSplitting`          | Aurora, RDS Multi-AZ DB Cluster                                                | Automatically routes each query to a reader or writer instance based on SQL analysis (SELECT to readers, DML/DDL to the writer), with optional `/*@reader*/`, `/*@writer*/`, and `/*@keep*/` routing hints. Requires the `sqlParser` plugin to be loaded before it.                                                                                                                                                                                                                                                                                                                                                                                             | [JSQLParser](https://github.com/JSQLParser/JSqlParser)                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 4.1.0                   |
...
| [GDB Read Write Splitting Plugin](./using-plugins/UsingTheGdbReadWriteSplittingPlugin.md)                            | `gdbReadWriteSplitting`           | Global Database                                                                | Enables read write splitting functionality where users can switch between reader and writer endpoints. Allows to specify home region and constrain new connections to this region.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | None                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 3.2.0                   |
| [Automatic Simple Read Write Splitting Plugin](./using-plugins/UsingTheAutoSimpleReadWriteSplittingPlugin.md)        | `autoSimpleReadWriteSplitting`    | Any database                                                                   | Automatically routes each query between the configured read and write endpoints based on SQL analysis (SELECT to the read endpoint, DML/DDL to the write endpoint), with `/*@reader*/`, `/*@writer*/`, and `/*@keep*/` hints. Requires the `sqlParser` plugin to be loaded before it.                                                                                                                                                                                                                                                                                                                                                                           | [JSQLParser](https://github.com/JSQLParser/JSqlParser)                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 4.2.0                   |
| [GDB Automatic Read Write Splitting Plugin](./using-plugins/UsingTheGdbAutoReadWriteSplittingPlugin.md)              | `gdbAutoReadWriteSplitting`       | Global Database                                                                | Automatically routes each query to a reader or writer instance based on SQL analysis, while constraining connections to the configured Global Database home region. Requires the `sqlParser` plugin to be loaded before it.                                                                                                                                                                                                                                                                                                                                                                                                                                     | [JSQLParser](https://github.com/JSQLParser/JSqlParser)                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 4.2.0                   |
| [GDB Simple Read Write Splitting Plugin](./using-plugins/UsingTheGdbSimpleReadWriteSplittingPlugin.md)               | `gdbSimpleReadWriteSplitting`     | Global Database                                                                | Switches between the configured read and write endpoints on `setReadOnly`, while applying Global Database home-region rules and Global Write Forwarding to the write endpoint.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | None                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 4.2.0                   |
| [GDB Automatic Simple Read Write Splitting Plugin](./using-plugins/UsingTheGdbAutoSimpleReadWriteSplittingPlugin.md) | `gdbAutoSimpleReadWriteSplitting` | Global Database                                                                | Automatically routes each query between the configured read and write endpoints based on SQL analysis, while applying Global Database home-region rules and Global Write Forwarding. Requires the `sqlParser` plugin to be loaded before it.                                                                                                                                                                                                                                                                                                                                                                                                                    | [JSQLParser](https://github.com/JSQLParser/JSqlParser)                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 4.2.0                   |
...
| [Custom Endpoint Plugin](./using-plugins/UsingTheCustomEndpointPlugin.md)                                            | `customEndpoint`                  | Aurora, <br>RDS Instance                                                       | Adds support for [Aurora custom endpoints](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Aurora.Endpoints.Custom.html) (`<custom-endpoint-name>.cluster-custom-<XYZ>.<region>.rds.amazonaws.com`). The plugin monitors custom endpoint membership so that new connections, failover, and read/write splitting only use instances that belong to the custom endpoint in use. <br><br> Note: an Aurora custom endpoint is not a user custom domain (CNAME alias); see `clusterInstanceHostPattern` for that case.                                                                                                                                  | [AWS Java SDK RDS v2.7.x or later](https://central.sonatype.com/artifact/software.amazon.awssdk/rds) and [valid AWS credentials](./AwsCredentials.md)                                                                                                                                                                                                                                                                                                                                                                               | 2.5.0                   |
...
```gradle
dependencies {
    implementation("software.amazon.jdbc:aws-advanced-jdbc-wrapper:4.4.1-SNAPSHOT")
}

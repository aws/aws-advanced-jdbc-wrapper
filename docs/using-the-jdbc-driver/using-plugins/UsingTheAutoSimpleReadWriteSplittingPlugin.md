...
On this axis, rotation means opening a fresh connection to `srwReadEndpoint` for each read, since that endpoint is the only reader target the plugin knows about. How widely reads actually spread therefore depends on what is behind that endpoint. An Aurora reader endpoint (`<cluster>.cluster-ro-<XYZ>.<region>.rds.amazonaws.com`) does distribute across the reader instances over time, but it may return the same instance for several consecutive lookups before moving to another one, so the spread is coarser and less even than what the topology-based plugins achieve with a reader host selector. A single-instance endpoint sends every read to the same host.

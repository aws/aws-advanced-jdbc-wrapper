...
> - :warning: If connecting with non-admin users, permissions must be granted to the users so that the blue/green metadata table can be properly queried. If the permissions are not granted, the metadata table will not be visible and blue/green plugin functionality will not work properly. Please see [Connecting with non-admin users](https://github.com/aws/aws-advanced-jdbc-wrapper/blob/main/docs/using-the-jdbc-driver/using-plugins/UsingTheBlueGreenPlugin.md#connecting-with-non-admin-users) section below.

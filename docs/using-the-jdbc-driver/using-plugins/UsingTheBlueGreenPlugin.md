...
| ~~`bgSuspendNewBlueConnections`~~ | Boolean |                              No                              | **Deprecated. New blue connection requests are always be suspended during switchover for versions v2.6.7+.** <br/><br/>Enables Blue/Green Deployment switchover to suspend new blue connection requests while the switchover process is in progress.                                                                                                                                                                                                    | `false`                  | `false`       |
...
2. If you're planning to connect with a non-admin user, add necessary permissions to allow the plugin to access Blue/Green metadata. Make sure that permissions are granted on **both** blue and green clusters. 

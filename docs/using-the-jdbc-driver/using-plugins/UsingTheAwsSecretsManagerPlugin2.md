...
The plugin is implemented as a thin subclass of the original plugin and reuses all of its credential fetching, parsing, caching, and error-handling logic. Only the cache-refresh strategy differs.
...
Login failure? ----NO----> Return connection
   ...
   YES (credentials rotated)
    |
    v
Synchronous re-fetch, retry connect
```

### Thundering Herd Prevention

When many connections are opened simultaneously with expired credentials, only **one** background refresh is triggered. All connections proceed immediately using the stale cached credentials. A `ConcurrentHashMap.compute()` pattern guarantees at most one in-flight refresh per `(secretId, region)` key. A login-failure re-fetch will also join an in-flight refresh rather than starting a new one.

### Cache Sharing

Both `awsSecretsManager` and `awsSecretsManager2` share the same credential cache (the driver's internal storage service). This means:
...
|----------------------------------------|-------------------------|:-------:|:--------------------------------------------------------:|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|:------------------------|---------------|
| `secretsManagerSecretId`               | 1.0.0 | String  |                           Yes                            | Set this value to be the secret name or the secret ARN.                                                                                                                                                                            | `secretId`              | `null`        |
| `secretsManagerRegion`                 | 1.0.0 | String  | Yes unless the `secretsManagerSecretId` is a Secret ARN. | Set this value to be the region your secret is in. When this parameter is not set, the region is taken from the region embedded in the `secretsManagerSecretId` ARN; if the secret ID is not an ARN, the connection fails. There is no fallback region. | `us-east-2`             | `null`        |
| `secretsManagerEndpoint`               | 2.3.0 | String  |                            No                            | Set this value to be the endpoint override to retrieve your secret from. This parameter value should be in the form of a URL, with a valid protocol (ex. `http://`) and domain (ex. `localhost`). A port number is not required.   | `http://localhost:1234` | `null`        |
| `secretsManagerExpirationTimeSec`      | 3.1.0 | Integer |                            No                            | The time in seconds that secrets are cached before they are considered stale. Stale secrets are still used for connections while a background refresh occurs. Minimum value: `300` (values below this are clamped with a warning). | `600`                   | `870`         |
| `secretsManagerSecretUsernameProperty` | 3.1.0 | String  |                            No                            | Set this value to be the key in the JSON secret that contains the username for database connection.                                                                                                                                | `writerUsername`        | `username`    |
| `secretsManagerSecretPasswordProperty` | 3.1.0 | String  |                            No                            | Set this value to be the key in the JSON secret that contains the password for database connection.                                                                                                                                | `readerPassword`        | `password`    |
| `secretsManagerConnectRetryTimeoutMs`  | 4.4.0 | Long    |                            No                            | Total time budget, in milliseconds, for retrying a connection that failed to log in, re-fetching the credentials before each retry. `0` disables retrying. See [Bridging a credential rotation window](./UsingTheAwsSecretsManagerPlugin.md#bridging-a-credential-rotation-window). | `90000`                 | `0`           |
| `secretsManagerConnectRetryIntervalMs` | 4.4.0 | Long    |                            No                            | Initial delay, in milliseconds, before a failed connection is retried. The delay doubles after every failed attempt, capped at 30000ms and at the remaining time budget. Only used when `secretsManagerConnectRetryTimeoutMs` is greater than `0`. | `2000`                  | `1000`        |

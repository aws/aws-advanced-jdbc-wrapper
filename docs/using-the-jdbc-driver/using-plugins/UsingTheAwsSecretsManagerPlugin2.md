...
| `secretsManagerConnectRetryTimeoutMs`  | Long    |                            No                            | Total time budget, in milliseconds, for retrying a connection that failed to log in, re-fetching the credentials before each retry. `0` disables retrying. See [Bridging a credential rotation window](./UsingTheAwsSecretsManagerPlugin.md#bridging-a-credential-rotation-window). | `90000`                 | `0`           |

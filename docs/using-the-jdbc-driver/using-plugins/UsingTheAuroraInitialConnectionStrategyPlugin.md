...
The Aurora Initial Connection Strategy Plugin allows users to configure their initial connection strategy, and it can
also be used to obtain a connection more reliably if DNS is updating by replacing an out of date endpoint. When the
...
be connecting to an old node. This plugin helps by replacing the out of date endpoint if DNS is updating.

...
| `iamHost`        | String  |    No    | This property will override the default hostname that is used to generate the IAM token. The default hostname is derived from the connection string. This parameter is useful when using SSH tunneling is in use and the connection string uses `localhost` instead of an RDS endpoint to establish connections.       | `database.cluster-hash.us-east-1.rds.amazonaws.com` |

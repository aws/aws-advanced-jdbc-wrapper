...
```gradle
dependencies {
    implementation group: 'software.amazon.jdbc', name: 'aws-advanced-jdbc-driver', version: '1.0.0'
...
```bash
wget https://github.com/awslabs/aws-advanced-jdbc-wrapper/releases/download/1.0.0/aws-advanced-jdbc-driver-1.0.0.jar
```

Then, the following command adds the AWS JDBC Driver to the CLASSPATH:

```bash
export CLASSPATH=$CLASSPATH:/home/userx/libs/aws-advanced-jdbc-driver-1.0.0.jar
```
...
```kotlin
dependencies {
    implementation("software.amazon.jdbc:aws-advanced-jdbc-driver:1.0.0")
}
```

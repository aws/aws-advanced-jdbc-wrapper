...
In this tutorial, you will set up a Wildfly and String Boot application with the AWS Advanced JDBC Wrapper, and use the wrapper to execute some simple database operations.
...
└───wildfly
      └───modules
          ├───software
              ...
              │       │   └───aws-advanced-jdbc-wrapper-2.1.2.jar
      └───standalone
          ├───configuration
              ├───amazon
              │   └───standalone.xml
...
The file `MVP.java` contains the following:
```java
package example;
...
<datasource jndi-name="java:jboss/datasources/AWSWrapper" pool-name="AWSWrapper" enabled="true" use-java-context="true" statistics-enabled="${wildfly.datasources.statistics-enabled:${wildfly.statistics-enabled:false}}">
   <connection-url>jdbc:aws-wrapper:postgresql:/cluster-endpoint:5432/db</connection-url>
...
<module xmlns="urn:jboss:module:1.1" name="software.amazon.jdbc">
  ...
  <resources>
    <resource-root path="aws-advanced-jdbc-wrapper-2.1.2.jar"/>
...
3. You should see the Spring application making a connection to the database and fetching data from the MVP table.

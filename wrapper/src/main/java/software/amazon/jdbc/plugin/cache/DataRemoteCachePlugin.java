...
public class DataRemoteCachePlugin extends AbstractConnectionPlugin {
  ...
  private String getCacheQueryKey(String query) {
    ...
    try {
      ...
      int nameIndexEnd = userName.indexOf('@');
      if (nameIndexEnd > 0) {
        userName = userName.substring(0, nameIndexEnd);
      }
      LOGGER.finest("DB driver protocol " + pluginService.getDriverProtocol()
          ...
          + ", catalog: " + catalog + ", schema: " + schema + ", user: " + userName
      ...
      String[] words = {catalog, schema, userName, query};

...
/**
 * Base PreparedStatement wrapper that automatically encrypts values.
 * Subclasses provide database-specific encrypted value setting logic.
 */
public abstract class BaseEncryptingPreparedStatement implements PreparedStatement {

  private static final Logger LOGGER =
      Logger.getLogger(BaseEncryptingPreparedStatement.class.getName());

  protected final PreparedStatement delegate;
  protected final MetadataManager metadataManager;
  protected final EncryptionService encryptionService;
  protected final KeyManager keyManager;
  protected final SqlAnalysisService sqlAnalysisService;
  protected final String sql;
  protected final String cleanSql;

  // Cache for parameter index to column name mapping
  protected final Map<Integer, String> parameterColumnMapping = new ConcurrentHashMap<>();
  protected String tableName;
  protected boolean mappingInitialized = false;

  protected BaseEncryptingPreparedStatement(
      ...
      String sql) {
    LOGGER.finest(() -> String.format("BaseEncryptingPreparedStatement created for SQL: %s", sql));
  ...
  /**
   * Database-specific method to set encrypted bytes for a parameter.
   *
   * @param parameterIndex the parameter index
   * @param encryptedBytes the encrypted bytes to set
   */
  ...
  public void setString(int parameterIndex, String x) throws SQLException {
    ...
    if (encryptedValue instanceof byte[]) {
      ...
      LOGGER.finest(
          () ->
              String.format(
                  "BaseEncryptingPreparedStatement.setString: param=%d, encryptedLength=%d",
                  parameterIndex,
                  encBytes.length));

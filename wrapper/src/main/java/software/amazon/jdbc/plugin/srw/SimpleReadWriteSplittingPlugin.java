...
public class SimpleReadWriteSplittingPlugin extends AbstractConnectionPlugin
  ...
  public static final AwsWrapperProperty READ_ONLY_ENDPOINT =
      new AwsWrapperProperty(
          "readOnlyEndpoint",
          null,
          "The read-only endpoint that should be used to connect to a new reader host.");
  public static final AwsWrapperProperty READ_WRITE_ENDPOINT =
      new AwsWrapperProperty(
          "readWriteEndpoint",
          null,
  ...
  SimpleReadWriteSplittingPlugin(final PluginService pluginService, final Properties properties) {
    String writeEndpoint = READ_WRITE_ENDPOINT.getString(properties);
    if (StringUtils.isNullOrEmpty(writeEndpoint)) {
      throw new
          RuntimeException(
          Messages.get(
              "SimpleReadWriteSplittingPlugin.missingRequiredConfigParameter",
              new Object[] {READ_WRITE_ENDPOINT.name}));
    }
    this.pluginService = pluginService;
    this.properties = properties;
    this.writeEndpoint = writeEndpoint;
    this.readEndpoint = READ_ONLY_ENDPOINT.getString(properties);
  }
  ...
  private void updateInternalConnectionInfo() throws SQLException {
    ...
    // Only update internal connection info if connection is to the endpoint.
    if (isWriteEndpoint(currentHost) && this.pluginService.getHostRole(currentConnection) == HostRole.WRITER) {
      setWriterConnection(currentConnection, currentHost);
    } else if (isReadEndpoint(currentHost) && this.pluginService.getHostRole(currentConnection) == HostRole.READER){
  ...
  private void logAndThrowException(final String logMessage) throws SQLException {
    ...
    throw new SimpleReadWriteSplittingSQLException(logMessage);
  }

  private void logAndThrowException(final String logMessage, final SqlState sqlState)
      throws SQLException {
    LOGGER.severe(logMessage);
    throw new SimpleReadWriteSplittingSQLException(logMessage, sqlState.getState());
  }
  ...
  private void logAndThrowException(
    ...
    throw new SimpleReadWriteSplittingSQLException(logMessage, SqlState.CONNECTION_UNABLE_TO_CONNECT.getState(), cause);
  }
  ...
  private void getNewWriterConnection() throws SQLException {
    ...
    if (this.pluginService.getHostRole(conn) != HostRole.WRITER) {
      logAndThrowException(
  ...
  private void getNewReaderConnection() throws SQLException {
    ...
    if (conn == null || readEndpoint == null) {
      ...
      return;
    } else if (this.pluginService.getHostRole(conn) != HostRole.READER) {
      logAndThrowException(

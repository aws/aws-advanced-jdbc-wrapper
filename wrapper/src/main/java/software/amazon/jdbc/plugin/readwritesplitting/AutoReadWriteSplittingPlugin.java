...
public class AutoReadWriteSplittingPlugin extends ReadWriteSplittingPlugin {
  ...
  public <T, E extends Exception> T execute(
    ...
    // Routing is intentionally skipped while a transaction is in progress: switching the
    // connection mid-transaction would break transactional guarantees, and a read-only
    // transaction can safely keep running on the reader it started on.
    ...
    if (executeMethodNames.contains(methodName) && !pluginService.isInTransaction()) {
      try {

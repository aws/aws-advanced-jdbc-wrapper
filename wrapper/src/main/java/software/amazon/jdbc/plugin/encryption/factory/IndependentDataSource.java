...
public class IndependentDataSource implements DataSource {
    ...
    private static final Logger logger = LoggerFactory.getLogger(IndependentDataSource.class);
    ...
    public IndependentDataSource(PluginService pluginService, Properties connectionProperties) {
        ...
        logger.debug("IndependentDataSource configuration: PropertiesCount={}", 
    ...
    public Connection getConnection() throws SQLException {
        ...
        try {
            logger.debug("Connection request #{} - creating new independent connection via PluginService", requestId);
    ...
    public Connection getConnection(String username, String password) throws SQLException {
        ...
        try {
            logger.debug("Connection request #{} - creating new independent connection with provided credentials", requestId);
    ...
    private Connection createNewConnection(Properties props) throws SQLException {
        ...
        logger.debug("Creating new independent connection via PluginService");
        ...
        try {
            ...
            logger.info("Successfully created independent connection via PluginService in {}ms " +
                       "(total successful: {}, total failed: {})", 
            ...
            logger.error("Failed to create independent connection via PluginService after {}ms: {} " +
                        "(total successful: {}, total failed: {})", 
            ...
            logger.error("Connection creation error details: {}", errorDetails);
    ...
    public void logHealthStatus() {
        ...
        if (isHealthy()) {
            logger.info("IndependentDataSource health check: {}", status);
        } else {
            logger.warn("IndependentDataSource health check - UNHEALTHY: {}", status);
        }
    }
}
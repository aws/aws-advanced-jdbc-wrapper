...
public class AuditLogger {
    ...
    private static final Logger auditLogger = Logger.getLogger("software.amazon.jdbc.audit".getName()));
    ...
    public void logKeyCreation(String masterKeyArn, String description, boolean success, String errorMessage) {
        ...
        try {
            ...
            if (success) {
                auditLogger.info("KMS master key created successfully - ARN: %s, Description: %s",
                    sanitizeArn(masterKeyArn), sanitizeDescription(description));
            } else {
                auditLogger.warn("KMS master key creation failed - Description: %s, Error: %s",
    ...
    public void logDataKeyGeneration(String masterKeyArn, String keyId, boolean success, String errorMessage) {
        ...
        try {
            ...
            if (success) {
                auditLogger.info("Data key generated successfully - Master Key: %s, Key ID: %s",
                    sanitizeArn(masterKeyArn), sanitizeKeyId(keyId));
            } else {
                auditLogger.warn("Data key generation failed - Master Key: %s, Error: %s",
    ...
    public void logDataKeyDecryption(String masterKeyArn, String keyId, boolean success, String errorMessage) {
        ...
        try {
            ...
            if (success) {
                auditLogger.info("Data key decrypted successfully - Master Key: %s, Key ID: %s",
                    sanitizeArn(masterKeyArn), sanitizeKeyId(keyId));
            } else {
                auditLogger.warn("Data key decryption failed - Master Key: %s, Key ID: %s, Error: %s",
    ...
    public void logEncryption(String tableName, String columnName, String keyId, boolean success, String errorMessage) {
        ...
        try {
            ...
            if (success) {
                auditLogger.info("Data encrypted successfully - Table: %s, Column: %s, Key ID: %s",
                    sanitizeTableName(tableName), sanitizeColumnName(columnName), sanitizeKeyId(keyId));
            } else {
                auditLogger.warn("Data encryption failed - Table: %s, Column: %s, Key ID: %s, Error: %s",
    ...
    public void logDecryption(String tableName, String columnName, String keyId, boolean success, String errorMessage) {
        ...
        try {
            ...
            if (success) {
                auditLogger.info("Data decrypted successfully - Table: %s, Column: %s, Key ID: %s",
                    sanitizeTableName(tableName), sanitizeColumnName(columnName), sanitizeKeyId(keyId));
            } else {
                auditLogger.warn("Data decryption failed - Table: %s, Column: %s, Key ID: %s, Error: %s",
                    sanitizeTableName(tableName), sanitizeColumnName(columnName),
                    sanitizeKeyId(keyId), sanitizeErrorMessage(errorMessage));
            }
        } finally {
    ...
    public void logMetadataOperation(String operation, String tableName, String columnName,
        ...
        try {
            ...
            if (success) {
                auditLogger.info("Metadata operation completed - Operation: %s, Table: %s, Column: %s",
                    operation, sanitizeTableName(tableName), sanitizeColumnName(columnName));
            } else {
                auditLogger.warn("Metadata operation failed - Operation: %s, Table: %s, Column: %s, Error: %s",
                    operation, sanitizeTableName(tableName), sanitizeColumnName(columnName),
    ...
    public void logConfigurationChange(String configType, String details, boolean success, String errorMessage) {
        ...
        try {
            ...
            if (success) {
                auditLogger.info("Configuration changed successfully - Type: %s, Details: %s",
                    configType, sanitizeConfigDetails(details));
            } else {
                auditLogger.warn("Configuration change failed - Type: %s, Details: %s, Error: %s",
    ...
    public void logConnectionParameterExtraction(String strategy, String connectionType,
        ...
        try {
            ...
            if (success) {
                auditLogger.info("Connection parameter extraction successful - Strategy: %s, Type: %s",
                    strategy, connectionType);
            } else {
                auditLogger.warn("Connection parameter extraction failed - Strategy: %s, Type: %s, Error: %s",
    ...
    public void logIndependentConnectionCreation(String jdbcUrl, boolean success, String errorMessage,
        ...
        try {
            ...
            if (success) {
                if (usedFallback) {
                    auditLogger.warn("Independent connection created using fallback - URL: %s",
                        sanitizedUrl);
                } else {
                    auditLogger.info("Independent connection created successfully - URL: %s",
                        sanitizedUrl);
                }
            } else {
                auditLogger.error("Independent connection creation failed - URL: %s, Error: %s",
    ...
    public void logConnectionSharingFallback(String reason, String originalFailure, boolean isActive) {
        ...
        try {
            ...
            if (isActive) {
                auditLogger.error("CONNECTION SHARING FALLBACK ACTIVATED - Reason: %s, Original Failure: %s",
                ...
                auditLogger.info("Connection sharing fallback deactivated - Reason: %s",
    ...
    public void logConnectionHealthCheck(String dataSourceType, boolean isHealthy,
        ...
        try {
            ...
            if (isHealthy) {
                auditLogger.info("Connection health check passed - Type: %s, Success Rate: {:.2f}%, " +
                               "Successful: %s, Failed: %s",
                               dataSourceType, successRate * 100, successCount, failureCount);
            } else {
                auditLogger.warn("Connection health check failed - Type: %s, Success Rate: {:.2f}%, " +
                               "Successful: %s, Failed: %s",
                               dataSourceType, successRate * 100, successCount, failureCount);
            }
        } finally {

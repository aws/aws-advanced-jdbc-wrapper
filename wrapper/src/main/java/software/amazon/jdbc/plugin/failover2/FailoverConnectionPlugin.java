...
public class FailoverConnectionPlugin extends AbstractConnectionPlugin {
  ...
  protected void failoverWriter() throws SQLException {
    ...
    LOGGER.finest("Should skip topology refresh: " + shouldSkipTopologyRefresh);
    ...
    try {
      ...
      LOGGER.finest("Current writer host updated to: " + this.currentWriterHost.getHost());
  ...
  public Connection connect(
    ...
    if (!ENABLE_CONNECT_FAILOVER.getBoolean(props)) {
      ...
      if (this.failoverMode == STRICT_WRITER) {
        ...
        this.currentWriterHost = this.pluginService.getHosts().stream().filter(x -> x.getRole() == HostRole.WRITER)
            .findFirst()
            .orElse(null);
        LOGGER.finest("Current writer host is set to: " + (this.currentWriterHost != null ? this.currentWriterHost.getHost() : "unset"));
      }
      return conn;
    }
    ...
    this.currentWriterHost = this.pluginService.getHosts().stream().filter(x -> x.getRole() == HostRole.WRITER)
        .findFirst()
        .orElse(null);
    LOGGER.finest("Current writer host is: " + (this.currentWriterHost != null ? this.currentWriterHost.getHost() : "unset"));
    return conn;
  }
}

 ...
 * Subclasses provide database-specific byte retrieval logic.
 */
public abstract class BaseDecryptingResultSet implements ResultSet {
  ...
  protected final Map<String, ColumnEncryptionConfig> columnConfigCache = new ConcurrentHashMap<>();
  protected final Map<Integer, String> columnIndexToNameCache = new ConcurrentHashMap<>();
  protected String tableName;
  protected boolean metadataInitialized = false;

  protected BaseDecryptingResultSet(
      ResultSet delegate,
  ...
  /**
   * Database-specific method to retrieve encrypted bytes from a column by index.
   *
   * @param columnIndex the column index
   * @return the encrypted bytes, or null if not encrypted
   */
  protected abstract byte[] getEncryptedBytes(int columnIndex) throws SQLException;

  /**
   * Database-specific method to retrieve encrypted bytes from a column by label.
   *
  ...
  private Object decryptValueIfNeeded(int columnIndex, Object value, Class<?> targetType)
      throws SQLException {

...
public class ConnectionPluginChainBuilder {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ConnectionPluginChainBuilder.class.getName());
      ...
      new HashMap<String, ConnectionPluginFactory>() {
        {
          ...
          put(
              "failover2",
              new software.amazon.jdbc.plugin.failover2.FailoverConnectionPluginFactory());
  ...
  /**
   * The final list of plugins will be sorted by weight, starting from the lowest values up to the
   * highest values. The first plugin of the list will have the lowest weight, and the last one will
   * have the highest weight.
   */
  protected static final Map<Class<? extends ConnectionPluginFactory>, Integer>
  ...
  protected static final ConcurrentMap<
          Class<? extends ConnectionPluginFactory>, ConnectionPluginFactory>
  ...
  */
  ...
  public List<ConnectionPlugin> getPlugins(
    ...
    if (configurationProfile != null && configurationProfile.getPluginFactories() != null) {
      List<Class<? extends ConnectionPluginFactory>> pluginFactoryClasses =
          configurationProfile.getPluginFactories();
      ...
      for (final Class<? extends ConnectionPluginFactory> factoryClazz : pluginFactoryClasses) {
        ...
        final ConnectionPluginFactory factory =
            pluginFactoriesByClass.computeIfAbsent(
                factoryClazz,
                (key) -> {
                  try {
                    return WrapperUtils.createInstance(
                        factoryClazz, ConnectionPluginFactory.class, null, (Object) null);
      ...
      for (final String pluginCode : pluginCodeList) {
        if (!pluginFactoriesByCode.containsKey(pluginCode)) {
          throw new SQLException(
              Messages.get("ConnectionPluginManager.unknownPluginCode", new Object[] {pluginCode}));
        }

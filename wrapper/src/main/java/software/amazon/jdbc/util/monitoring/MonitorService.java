...
public interface MonitorService {
  /**
   * Registers a new monitor type with the monitor service. This method needs to be called before
   * adding new types of monitors to the monitor service, so that the monitor service knows when to
   * dispose of a monitor. Expected monitor types will be added automatically during driver
   * initialization, but this method can be called by users if they want to add a new monitor type.
   *
   * @param monitorClass the class of the monitor, eg `CustomEndpointMonitorImpl.class`.
   * @param expirationTimeoutNanos how long a monitor should be stored without use before being
   *     considered expired, in nanoseconds. Expired monitors may be removed and stopped.
   * @param heartbeatTimeoutNanos a duration in nanoseconds defining the maximum amount of time that
   ...
   * @param errorResponses a {@link EnumSet} defining actions to take if the monitor is stuck or in
   *     an error state.
  ...
  /**
   * Creates and starts the given monitor if it does not already exist and stores it under the given
   * monitor type and key. If the monitor already exists, its expiration time will be renewed, even
   * if it was already expired.
   *
   ...
   * @param servicesContainer the source service container for the monitor. The new monitor will
   *     create its own service container but will reuse some components from the source container.
   * @param originalProps the properties of the original database connection.
   * @param initializer an initializer function to use to create the monitor if it does not already
   *     exist.
   * @param <T> the type of the monitor.
   ...
   * @param defaultConnectionProvider the connection provider to use to create new connections if
  ...
  /**
   * Removes the monitor stored at the given key. If the expected monitor class does not match the
   * actual monitor class no action will be performed.
   *
   * @param monitorClass the expected class of the monitor.
   * @param key the key for the monitor.
   * @param <T> the type of the monitor.
   * @return the monitor that was removed. Returns null if there was no monitor at the given key or
   *     the expected monitor class did not match the actual monitor class.
   */
  @Nullable
  ...
  /**
   * Releases any resources opened by the monitor service, stops all monitors, and removes all
   * monitors from the monitor service.
   */

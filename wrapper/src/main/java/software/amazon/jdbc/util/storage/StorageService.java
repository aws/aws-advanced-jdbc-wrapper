...
public interface StorageService {
  /**
   * Registers a new item class with the storage service. This method needs to be called before
   * adding new classes of items to the service, so that the service knows when and how to dispose
   * of the item. Expected item classes will be added automatically during driver initialization,
   * but this method can be called to add new classes of items.
   *
   * @param itemClass the class of the item that will be stored, eg `CustomEndpointInfo.class`.
   * @param isRenewableExpiration controls whether the item's expiration should be renewed if the
   *     item is fetched, regardless of whether it is already expired or not.
   * @param timeToLiveNanos how long an item should be stored before being considered expired, in
   *     nanoseconds.
   * @param shouldDisposeFunc a function defining whether an item should be disposed if expired. If
   *     null is passed, the item will always be disposed if expired.
   ...
   *
   * @param key the key for the item, eg
  ...
  /**
   * Clears all items of the given item class. For example,
   * storageService.clear(AllowedAndBlockedHosts.class) will remove all AllowedAndBlockedHost items

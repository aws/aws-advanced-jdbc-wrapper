...
public class AuroraMysqlAwsIamIntegrationTest extends AuroraMysqlBaseTest {
  /**
  ...
  void test_AwsIam_NoAwsProtocolConnection() throws SQLException {
    ...
    final Properties invalidProp = initAwsIamProps("WRONG_" + AURORA_MYSQL_DB_USER + "_USER", AURORA_MYSQL_PASSWORD);
  ...
  void test_AwsIam_UserInConnStr() throws SQLException {
    ...
    final Connection validConn = DriverManager.getConnection(dbConn + "?" + PropertyDefinition.USER.name + "=" + AURORA_MYSQL_DB_USER, awsIamProp);
    Assertions.assertNotNull(validConn);
    Assertions.assertThrows(
        SQLException.class,
        () -> DriverManager.getConnection(dbConn + "?" + PropertyDefinition.USER.name + "=WRONG_" + AURORA_MYSQL_DB_USER, awsIamProp)
    );
  }
}

...
class SqlAnalyzerTest {
  ...
  public void testMultiTableJoin() {
    ...
    assertTrue(result.columns.stream().anyMatch(c -> "products".equals(c.tableName) && "title".equals(c.columnName)));
  }
  ...
  public void testSelectWithCast() {
    SQLAnalyzer.QueryAnalysis result = analyzer.analyze("SELECT CAST(price AS INTEGER) FROM products");
    assertEquals("SELECT", result.queryType);
    assertTrue(result.tables.contains("products"));
  }

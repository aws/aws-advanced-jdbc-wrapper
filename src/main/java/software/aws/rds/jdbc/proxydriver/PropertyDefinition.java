...
public class PropertyDefinition {
    ...
    public static final ProxyDriverProperty PLUGIN_FACTORIES = new ProxyDriverProperty(
            "proxyDriverPluginFactories",
            null,
            "Coma separated list of connection plugin factories");

    public static final ProxyDriverProperty LOGGER_LEVEL = new ProxyDriverProperty(
    ...
    private static final Map<String, ProxyDriverProperty> PROPS_BY_NAME = new HashMap<String, ProxyDriverProperty>();

    static {
        PROPS_BY_NAME.clear();
        Field[] ff = PropertyDefinition.class.getDeclaredFields();
        Arrays.stream(PropertyDefinition.class.getDeclaredFields())
                .filter(f -> f.getType() == ProxyDriverProperty.class

...
### :magic_wand: Added
- Added a unified read/write splitting plugin family built on a composition-based plugin, preserving the existing `readWriteSplitting`, `autoReadWriteSplitting`, `srw`, and `gdbReadWriteSplitting` codes while adding the new `autoSimpleReadWriteSplitting`, `gdbAutoReadWriteSplitting`, `gdbSimpleReadWriteSplitting`, and `gdbAutoSimpleReadWriteSplitting` codes, family-wide query-level load balancing (`queryLevelLoadBalancing`, `loadBalancingIncludeWriter`), and statement recreation on connection switch (`allowStatementRecreationOnConnectionSwitch`) ([PR #2016](https://github.com/aws/aws-advanced-jdbc-wrapper/pull/2016), [documentation](https://github.com/aws/aws-advanced-jdbc-wrapper/blob/main/docs/using-the-jdbc-driver/using-plugins/UsingTheReadWriteSplittingPlugin.md)).
- Added XA distributed transaction support through the new `AwsWrapperXADataSource`, which wraps a target driver `XADataSource` so the plugin pipeline applies to the application-facing connection while the XA control surface is coordinated by a JTA transaction manager ([PR #2045](https://github.com/aws/aws-advanced-jdbc-wrapper/pull/2045), [documentation](https://github.com/aws/aws-advanced-jdbc-wrapper/blob/main/docs/using-the-jdbc-driver/UsingXADataSource.md)).
...
- Fixed transaction detection missing `BEGIN`/`COMMIT` hidden behind `--` and `#` line comments, and the read/write splitting gate now pins the connection when the autocommit state cannot be read ([PR #2056](https://github.com/aws/aws-advanced-jdbc-wrapper/pull/2056), [documentation](https://github.com/aws/aws-advanced-jdbc-wrapper/blob/main/docs/using-the-jdbc-driver/using-plugins/UsingTheReadWriteSplittingPlugin.md)).
...
- Fixed a HikariCP `PoolInitializationException` aborting failover instead of being rethrown as a `SQLException` and retried by the failover loop ([PR #2051](https://github.com/aws/aws-advanced-jdbc-wrapper/pull/2051), [documentation](https://github.com/aws/aws-advanced-jdbc-wrapper/blob/main/docs/using-the-jdbc-driver/using-plugins/UsingTheFailoverPlugin.md)).
...
- Fixed IAM tokens being signed for the wrong region after a region change, such as a cross-region failover, by caching `RdsUtilities` per region ([PR #2078](https://github.com/aws/aws-advanced-jdbc-wrapper/pull/2078), [documentation](https://github.com/aws/aws-advanced-jdbc-wrapper/blob/main/docs/using-the-jdbc-driver/using-plugins/UsingTheIamAuthenticationPlugin.md)).
...
- Softened the JAXP `ACCESS_EXTERNAL_DTD` and `ACCESS_EXTERNAL_SCHEMA` settings in `CachedSQLXML#getSource` to best-effort so cached XML reads work on classpaths with a standalone Xerces, while `disallow-doctype-decl` stays strict ([PR #2095](https://github.com/aws/aws-advanced-jdbc-wrapper/pull/2095), [documentation](https://github.com/aws/aws-advanced-jdbc-wrapper/blob/main/docs/using-the-jdbc-driver/using-plugins/UsingTheRemoteQueryCachePlugin.md#xml-columns)).

...
import software.aws.rds.jdbc.proxydriver.plugin.AbstractConnectionPlugin;
import software.aws.rds.jdbc.proxydriver.util.ConnectionUrlParser;
import software.aws.rds.jdbc.proxydriver.util.Messages;
import software.aws.rds.jdbc.proxydriver.util.RdsUrlType;
import software.aws.rds.jdbc.proxydriver.util.RdsUtils;
import software.aws.rds.jdbc.proxydriver.util.SqlState;
import software.aws.rds.jdbc.proxydriver.util.Utils;
...
public class FailoverConnectionPlugin extends AbstractConnectionPlugin {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(FailoverConnectionPlugin.class.getName());
  ...
  public static final ProxyDriverProperty FAILOVER_CLUSTER_TOPOLOGY_REFRESH_RATE_MS = new ProxyDriverProperty(
  ...
  public static final ProxyDriverProperty FAILOVER_TIMEOUT_MS = new ProxyDriverProperty(
  ...
  public static final ProxyDriverProperty FAILOVER_WRITER_RECONNECT_INTERVAL_MS = new ProxyDriverProperty(
  ...
  public static final ProxyDriverProperty FAILOVER_READER_CONNECT_TIMEOUT_MS = new ProxyDriverProperty(
      "failoverReaderConnectTimeoutMs", "30000",
      "Reader connection attempt timeout during a reader failover process.");

  public static final ProxyDriverProperty AUTO_RECONNECT = new ProxyDriverProperty(
      "wrapperAutoReconnect", "false",
  ...
  public static final ProxyDriverProperty AUTO_RECONNECT_FOR_POOLS = new ProxyDriverProperty(
      "wrapperAutoReconnectForPools", "false",
  ...
  public <T, E extends Exception> T execute(
    ...
    try {
      ...
      if (type.isRdsCluster()) {
        ...
        LOGGER.finer(
            Messages.get("Failover.parameterValue", new Object[] {"explicitlyReadOnly", this.explicitlyReadOnly}));
      }
  ...
  public void initHostProvider(
    ...
    initHostProvider(
        ...
        (hostListProvider) -> new ClusterAwareReaderFailoverHandler(
            hostListProvider,
            this.pluginService,
            this.properties,
            this.failoverTimeoutMsSetting,
            this.failoverReaderConnectTimeoutMsSetting),
        (hostListProvider) -> new ClusterAwareWriterFailoverHandler(
            hostListProvider,
            this.pluginService,
  ...
  private void initSettings() {
    ...
    this.failoverTimeoutMsSetting =
        FAILOVER_TIMEOUT_MS.getInteger(this.properties);
    this.failoverClusterTopologyRefreshRateMsSetting =
  ...
  private boolean shouldAttemptReaderConnection() {
    return isExplicitlyReadOnly() && this.pluginService.getHosts() != null && this.pluginService.getHosts().size() > 1;
  }
   ...
   * @throws SQLException if an error occurs
   */
  private void switchCurrentConnectionTo(int hostIndex, Connection connection)
      throws SQLException {
   ...
   * @throws SQLException if an error occurs
   */
  private Connection createConnectionForHostIndex(int hostIndex)
      throws SQLException {
   ...
   * @throws SQLException if an error occurs
   */
  protected Connection createConnectionForHost(HostSpec baseHostSpec)
      throws SQLException {
